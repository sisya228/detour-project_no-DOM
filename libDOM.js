const positionElement = {
  ...
  getRelativeParentElement: function(elm, pElm){
    ...
    var obj = pElement != document.body? {
...
function getContactObjects(objA, objB, crossroads) {
...
function contactObjects(objA, objB){
  return getContactObjects(objA, objB) || getContactObjects(objB, objA) 
    ||
    getContactObjects(objA, objB, true) ||     getContactObjects(objB, objA, true);
}
...
const MoveFromAToB = function(leftA, topA, leftB, topB, move, targetObject){
    ...
    this.setCoordinates = function(leftA, topA, leftB, topB, move, targetObject){
      var moveObj = this.getNumberPx(move);
        ...
        var ifStop;
        if (this.moveFunction && this.numStart < 2) {
          this.numStart++;
          this.moveFunction(this, this.elm, [...this.arg]);
        }
        if (this.numStart > 1) {
          this.stop("error");
          console.log("move stop!"); //system info
        }
        this.numStart = 0;
              ...
              if(this.nextCoordinates){
                      if(this.nextCoordinates[this.j]){
                       if(this.functionNextCoordinates){
                         this.functionNextCoordinates(this, this.elm, [...[...this.nextCoordinates[this.j]]]);
                       }
    ...
    this.start = function(interval){
        ...
        if (this.introFunction && !interval && this.startPosition && this.moveFunction) {
          this.introFunction(this, this.elm, [...this.arg]);
        }
        if (interval) {
    ...
    this.stop = function(interior){
        ...
        if(this.endFunction && (!this.looping || !interior) && this.numStop < 2 && interior !="error"){
          this.endFunction(this, this.elm, [...this.arg]);
          if (interior == "error") {
            console.log("move stop"); // system info
          }
        }
...
class MovieElement {
  constructor(elm) {
    ...
    this.getPosition = positionElement.getRelativeParentElement.bind(positionElement);
  ...
  setMovie(){
    ...
    var moveY = this.moveY;
      ...
      this.moveActive = (this.moveActive || !this.fixedElm) && this.moveF(moveX, moveY, ...this.argFuncMove);
      if(this.moveActive) {
...
class MovieElementForMouse {
  constructor(elm) {
    this.elm = elm;
    this.getPosition = positionElement.getRelativeParentElement.bind(positionElement);
    this.position = positionElement.getRelativeParentElement(elm);
    this.x = this.position.left;
    this.y = this.position.top;
    this.setPositionElement(this.elm, this.x, this.y);
    this.startX;
    ...
    this.statusMoveY = true;
    this.indexTouches = 0;
    this.movieElementForMouse = false;
    
  }
  setStart(){
    this.elm.addEventListener("mousedown", evn => {
      //this.indexTouches = evn.touches.length - 1;
      this.startX = evn.clientX;
      this.startY = evn.clientY;
      this.x = parseInt(evn.currentTarget.style.left);
      this.y = parseInt(evn.currentTarget.style.top);
      this.startF(this.x, this.y, ...this.argFuncStart);
      this.movieElementForMouse = true;
    });
  };
  setMovie(){
    this.elm.parentElement.addEventListener("mousemove", evn => {
      positionElement.setPosition = false;
      var positionContainer = positionElement.getRelativeDocument(evn.currentTarget);
      positionElement.setPosition = true;
      if (this.movieElementForMouse) {
        var moveX = this.moveX;
        var moveY = this.moveY;
        if(this.statusMoveX) {
            moveX = evn.pageX-positionContainer.left;//
        }
        if(this.statusMoveY) {
            moveY = evn.pageY-positionContainer.top;//
         }
        this.moveActive = (this.moveActive || !this.fixedElm) && this.moveF(moveX, moveY, ...this.argFuncMove);
        if(this.moveActive) {
            this.setPosition(evn, moveX, moveY);
            ...
            this.moveX = parseInt(this.elm.style.left);
            this.moveY = parseInt(this.elm.style.top);
            this.moveStop = true;
        }
      }
     });
...
setEnd(){
    this.elm.addEventListener("mouseup", evn => {
      this.funcMouseUpAndOut(this.elm);
    });
    // this.elm.parentElement.addEventListener("mouseout", evn => {
    //   this.funcMouseUpAndOut(this.elm);
  ...
  funcMouseUpAndOut(elm){
    this.movieElementForMouse = false;
    elm.style.left = this.moveX+"px";
    elm.style.top = this.moveY+"px";
    this.x = this.moveX;
    this.y = this.moveY;
    this.endF(this.x, this.y, ...this.argFuncEnd);
    this.moveActive = true;
  ...
  // getElementCoordinates(evn, i){
  //   var mouseV = [evn.layerX, evn.layerY];
  //   var startV = [this.startX, this.startY];
  //   var distance = mouseV[i] - startV[i];
  //   var v = [this.x, this.y];
  //   return distance + v[i];
