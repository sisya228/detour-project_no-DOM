class Detour {
  ...
  getCoordinates(startElm, endElm) {
    ...
    //this.blockElm.style.width = 0+'px';
    //this.blockElm.style.height = 0+'px';
...
window.onload = () => {
  ...
  myClass.onclick = () => {
    ...
    var detour = new Detour("obstacles", {width: '300px', height: '300px'});
    detour.L = 10;
    //detour.ITERATION = 0;
    positionElement.getRelativeParentElement(stElm);
    let coordinateStart = stElm.style;
    console.log(coordinateStart.top);
    let coordinateEnd;
    var coordinates = detour.getCoordinates(coordinateStart, targElm);
